...
async function getAccountSettings(req: Request, res: Response, next: any) {
    try {
        ...
        settings.EmailAddresses.forEach(item => {
...
async function addAccountEmail(req: Request, res: Response, next: any) {
    ...
    try {
        ...
        if (alreadyExists) return res.status(400).json({ message: 'accountEmail already exists.' });

        accountEmail.accountId = token.accountId;
        const result = await accountEmailRepository.add(accountEmail);
        if (!result.id) return res.status(400).json("Couldn't save the accountEmail.");
...
async function getAccountEmail(req: Request, res: Response, next: any) {
    try {
        ...
        if (!accountId || !accountEmailId) return res.status(400).json({ message: 'Both ids are required' });
        ...
        if (!settings || settings.length === 0) return res.status(404).json({ message: 'settings not found' });
...
async function setAccountEmail(req: Request, res: Response, next: any) {
    try {
        ...
        const updatedAccountEmail = await accountEmailRepository.set(accountEmailId, token.accountId, accountEmailParams);
