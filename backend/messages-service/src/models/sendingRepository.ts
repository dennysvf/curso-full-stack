...
async function findQueuedOne(id: string, messageId: number, accountId: number, contactId: number) {
    try {
        const sending = sendingModel.findOne<ISendingModel>({ where: { id, contactId, messageId, accountId, status: SendingStatus.QUEUED } });
        return sending;
    }
    catch (error) {
...
function removeById(sendingId: string, accountId: number) {
